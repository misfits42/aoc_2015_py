...
class Spell(Enum):
    ...
    @staticmethod
    def list():
        return list(map(lambda c: c.value, Spell))
...
class Player(Entity):
    ...
    def cast_spell(self, spell, enemy):
        ...
        if self.mana < spell.get_mana_cost() or spell in self.effects:
            return False
        ...
        match spell:
            ...
            case Spell.POISON:
                self.effects[spell, spell.get_effect_turns()]
            case Spell.RECHARGE:
                self.effect[spell, spell.get_effect_turns()]

    def process_effects(self, enemy):
        effects_to_remove = []
        for (effect, turns_remaining) in self.effects:
            turns_remaining -= 1
            match effect:
                case Spell.SHIELD:
...
def conduct_fight_recursive(player, boss, mana_expended):
    ()
